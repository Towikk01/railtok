...
const DynamicBlock = ({ title, description, isLast = true }) => {
	return (
		<div className={`flex ${title ? 'md:flex-col md:gap-9' : 'md:flex-col md:gap-2'} items-start flex-row   last:mb-0 gap-[22px] w-fit`}>
			<div className={`flex flex-col ${title ? 'md:flex-col' : 'md:flex-row'} items-center gap-[1.15rem] md:gap-5 md:w-full`}>
				<div
			...
			<div className={'flex flex-col gap-4'}>
				...
				<p className={`${title ? 'text-[21px]' : 'text-[26px]'} md:text-[20px] text-white  md:leading-[26px] text-white/95 font-normal leading-[22px]  md:mb-0 md:mr-[30px] lg:mr-0 lg:max-w-[306px] md:max-w-[163px]`}>
					{description}
				</p>
			</div>
		</div>
